...
public class ModRecipeProvider extends RecipeProvider implements IConditionBuilder {
    ...
    protected void buildRecipes(RecipeOutput pRecipeOutput) {
        ShapedRecipeBuilder.shaped(RecipeCategory.MISC, ModBlocks.AQUAMARINE_BLOCK.get())
                ...
                .pattern("BBB")
                .pattern("BBB")
                .define('B', ModItems.AQUAMARINE.get())
                .unlockedBy("has_block_aquamarine", has(ModItems.AQUAMARINE.get())).save(pRecipeOutput);

        ShapelessRecipeBuilder.shapeless(RecipeCategory.MISC, ModItems.AQUAMARINE.get(), 9)
                .requires(ModBlocks.AQUAMARINE_BLOCK.get())
                .unlockedBy("has_aquamarine_block", has(ModBlocks.AQUAMARINE_BLOCK.get())).save(pRecipeOutput);
    }
    ...
    protected static <T extends AbstractCookingRecipe> void oreCooking(RecipeOutput pRecipeOutput, RecipeSerializer<T> pCookingSerializer, AbstractCookingRecipe.Factory<T> factory,
        ...
        for (ItemLike itemlike : pIngredients) {
            SimpleCookingRecipeBuilder.generic(Ingredient.of(itemlike), pCategory, pResult, pExperience, pCookingTime, pCookingSerializer, factory).group(pGroup).unlockedBy(getHasName(itemlike), has(itemlike))
                    .save(pRecipeOutput, AquamarineTools.MOD_ID + ":" + getItemName(pResult) + pRecipeName + "_" + getItemName(itemlike));
        }
    }
}